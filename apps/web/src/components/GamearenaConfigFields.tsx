...
export function GamearenaConfigFields({
  ...
  const strategySection = (
    <section className="config-section">
      <h4 className="config-section-title">Strategy</h4>
      ...
      {!compact && strategy !== "random" && (
        <p className="config-section-note muted">{strategyHint}</p>
      )}
    </section>
  );

  const refillsSection = showOffchain ? (
    <section className="config-section">
    ...
    <section className="config-section">
      ...
      {showOffchain && (
        <ConfigField label="Pause between rounds (live spectator)">
          ...
          {!compact && (
            <p className="config-section-note muted">
              Wait after each throw so GameArena&apos;s SSE feed plays out (2500
              ms for comfortable spectating; 1000 ms minimum).
            </p>
          )}
        </ConfigField>
      )}
  ...
  const onchainSection = showOnchain ? (
    <section className="config-section">
  ...
  return (
    <div
      className={`gamearena-config-form${variant === "onboard" ? " gamearena-config-form--onboard" : ""}`}
    >
      {playModeSection}
      {strategySection}
      {compact ? (
        <details className="deploy-advanced-details">
          <summary>Limits & refills</summary>
