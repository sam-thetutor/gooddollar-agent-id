import type { SkillConfiguration } from "./host.js";
...
export const CHESS_ARENA_PLAY_MODES = [
  ...
  { id: "accept", label: "Accept only", hint: "Only join existing open lobbies — skip when none are available." },
  { id: "open", label: "Open only", hint: "Always create a new lobby and wait for opponents." },
] as const;
...
export function chessArenaPlayModeLabel(config: SkillConfiguration): string {
  const mode = parseChessArenaPlayMode(config);
  return CHESS_ARENA_PLAY_MODES.find((m) => m.id === mode)?.label ?? mode;
}

export function chessArenaSolverLabel(config: SkillConfiguration): string {
  const engine = parseChessArenaSolverEngine(config);
  return CHESS_ARENA_SOLVER_ENGINES.find((e) => e.id === engine)?.label ?? engine;
}
