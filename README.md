...
**GoodRemind** (**[@gooddollarremind_bot](https://t.me/gooddollarremind_bot)**) is a free Telegram assistant that helps GoodDollar claimers never miss their daily UBI: start the assistant, send any Celo wallet address you claim with (you can add several), and it watches your eligibility on-chain and pings you when G$ is ready to claim. No seed phrases, no private keys, read-only chain checks only. 

Use :

- `/status` anytime for a live snapshot, 
- `/list` to see watched wallets, and `/remove` or 
- `/stop` if you want to unsubscribe; 
...
- `/status` anytime for a live snapshot, 
- `/list` to see watched wallets,  
- `/remove` or `/stop` if you want to unsubscribe; 

it’s built for the GoodDollar community on Telegram and takes less than a minute to set up, so feel free to share the link in your groups and channels so more people can turn on daily reminders.
